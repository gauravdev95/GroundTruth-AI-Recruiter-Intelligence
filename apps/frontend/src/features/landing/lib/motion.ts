...
/**
 * Motion infrastructure for the landing page.
 *
 * Framer Motion is the only animation library on this page. GSAP, ScrollTrigger
 * and Lenis were removed: two libraries contending for the same `transform`
 * property is a correctness problem before it is a bundle problem, and Lenis'
 * wheel smoothing is the damped form of the scroll-jacking this page refuses.
 *
 * Four rules hold everywhere in here:
 *
 * 1. Only `transform` and `opacity` are ever animated.
 * 2. `prefers-reduced-motion: reduce` is a real code path, subscribed rather
 *    than sampled once — every hook below resolves to its final state when it
 *    is set, and reacts if the user changes it mid-session.
 * 3. There is exactly one `IntersectionObserver` per distinct set of observer
 *    options, and exactly one `requestAnimationFrame` loop, page-wide.
 * 4. Nothing survives unmount. Every subscription returns its own teardown.
 */

/* ==================================================================
   TOKENS — no animation anywhere on this page uses a literal
   ================================================================== */
...
export const EASE: Record<"entrance" | "standard" | "exit", Bezier> = {
  /** Decelerating. Every reveal on the page uses this one. */
...
export const STAGGER = {
  ...
  pipelineStage: 0.4,
  /**
   * The hero console's beat. Twelve beats at this interval is ~1.9s of
   * choreography, which has to fit inside the ten-second budget a stranger
   * gives the page *and* start after `useAfterIdle` has let first paint
   * through. A slower beat would push the score ring past both.
   */
  consoleBeat: 0.16,
} as const;
...
/**
 * The single source of truth for reduced motion. Every animated component on
 * the page consumes this hook; nothing writes its own media query.
 */
...
/**
 * Imperative read, for the two places that need the value outside React's
 * render cycle (the shared rAF loop and the magnetic pointer handler).
 * Components must use `useReducedMotionSafe` instead — this does not subscribe.
 */
...
/* ==================================================================
   SHARED rAF LOOP — one clock for the whole page
   ================================================================== */
...
if (typeof document !== "undefined") {
  // Ambient loops must not burn frames in a background tab.
...
/**
 * Adds a callback to the page's single rAF loop. The loop starts on the first
 * subscription and stops entirely when the last one leaves, so an idle page
 ...
 * observer options share one `IntersectionObserver` instance, so a page with
 * forty reveals allocates one observer rather than forty.
 */
 ...
 * content on this page may depend on an observer firing to become visible.
 */
export function useInViewShared<T extends Element = HTMLDivElement>(
 ...
 *
 * Used to keep an autoplaying scene from competing with first paint. The hero's
 * sequence mounts and unmounts several layers in its first two seconds, and
 * starting that at mount put all of it inside the window that decides LCP and
 * Total Blocking Time — the page was measurably slower for animating while it
 * was still trying to render.
 */
 ...
 *
 * Deliberately library-free: this runs on the primary CTA above the fold, and
 * the release is a CSS transition so letting go costs no frames. Pointer reads
 * are throttled to one per frame on the shared loop.
 */
