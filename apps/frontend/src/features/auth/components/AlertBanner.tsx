 ...
 */
export function AlertBanner({ message, tone = "app" }: { message: string; tone?: FieldTone }) {
  return (
    <div
      role="alert"
      aria-live="assertive"
      className={
        tone === "hero"
          ...
          : "flex items-start gap-2.5 rounded-[var(--r-md)] border border-[var(--failed)]/30 bg-[var(--failed)]/10 px-4 py-3 text-sm text-[var(--failed)] animate-fade-in"
      }
    >
