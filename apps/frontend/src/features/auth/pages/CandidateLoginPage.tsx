...
export function CandidateLoginPage() {
  ...
  const onSubmit = handleSubmit((values) => {
    setFormError(null);
    login.mutate(
      { ...values, expected_role: "candidate" },
      {
        onSuccess: () => navigate("/"),
        onError: (error) => {
          if (isAxiosError(error) && (error.response?.data as { code?: string } | undefined)?.code === "EMAIL_NOT_VERIFIED") {
            navigate(`/verify-email?email=${encodeURIComponent(values.email)}&role=candidate`);
            return;
          }
  ...
  return (
    <AuthLayout
      title="Welcome back"
      subtitle="Log in to continue your candidate journey."
      footer={
        <>
          New to GroundTruth AI?{" "}
          <Link to="/signup/candidate" className="font-medium text-indigo-600 hover:text-indigo-500">
            Create an account
          </Link>
        </>
      }
    >
      <form onSubmit={onSubmit} noValidate className="flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/forgot-password?role=candidate" className="text-sm font-medium text-indigo-600 hover:text-indigo-500">
            Forgot password?
          </Link>
        </div>
