  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs ${
        reason.candidate_has_skill ? "border-[var(--verified)]/30 bg-[var(--verified)]/10 text-[var(--verified)]" : "border-[var(--rule)] bg-[var(--panel)] text-[var(--muted)]"
      }`}
      title={
...
export function MatchedCandidatesList({ candidates }: { candidates: MatchedCandidate[] }) {
  if (candidates.length === 0) {
    return (
      <p className="rounded-2xl border border-dashed border-[var(--rule)] bg-[var(--panel)] px-4 py-8 text-center text-sm text-[var(--slate)]">
  ...
  return (
    <ul className="space-y-3">
      {candidates.map((match) => (
        <li key={match.candidate.candidate_profile_id} className="rounded-2xl border border-[var(--rule)] bg-[var(--panel)] p-5">
          ...
          <div className="mb-3 flex flex-wrap gap-3 text-xs text-[var(--slate)]">
          ...
          {match.matched_required_skills.length > 0 ? (
            <div className="mb-2">
              <p className="mb-1 text-xs font-medium uppercase tracking-wide text-[var(--muted)]">Must-have</p>
          ...
          {match.matched_desirable_skills.length > 0 ? (
            <div>
              <p className="mb-1 text-xs font-medium uppercase tracking-wide text-[var(--muted)]">Desirable</p>
              <div className="flex flex-wrap gap-1.5">
