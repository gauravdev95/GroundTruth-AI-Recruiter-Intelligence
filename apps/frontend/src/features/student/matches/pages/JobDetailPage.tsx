      ...
      <div className="flex min-w-0 items-start gap-2.5">
        <span
          className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full ${
            has ? "bg-verified/10 text-verified" : "bg-slate-100 text-slate-400"
          }`}
        >
        ...
        <div className="min-w-0">
          <p className="text-[13px] font-medium text-ink">
            {skill.skill_name}
            {!isRequired ? <span className="ml-1.5 text-[11px] text-slate-400">nice to have</span> : null}
          </p>
          {has && source ? (
            <p className="truncate text-[11px] text-slate-400">{source.title ?? source.type}</p>
          ) : !has ? (
            <p className="text-[11px] text-slate-400">No verified evidence on your profile yet</p>
          ) : null}
        </div>
      </div>
      {percent !== null ? (
...
export function JobDetailPage() {
  ...
  if (!match) {
    return (
      <ErrorState
        ...
        action={
          <Link to="/student/matches" className="text-sm font-medium text-ink hover:underline">
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <Link
        to="/student/matches"
        className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-500 transition hover:text-ink"
      >
        <ArrowLeft size={13} aria-hidden="true" />
        Your matches
      </Link>

      <header className="rounded-xl border border-rule bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="shrink-0 text-right">
            <p className="tabular font-display text-3xl font-semibold text-gt-electric">
              {displayScore}%
            </p>
        ...
        <p className="mt-4 border-l-2 border-gt-electric bg-gt-electric/5 py-2.5 pl-3.5 pr-3 text-[13px] leading-relaxed text-slate-700">
          {match.reasoning}
        </p>
      ...
      <section className="rounded-xl border border-rule bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
