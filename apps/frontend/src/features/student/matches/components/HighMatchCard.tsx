...
export function HighMatchCard({ match, hasApplied }: { match: MatchedJob; hasApplied: boolean }) {
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-[var(--blue)]/25 bg-[var(--panel)] shadow-[0_1px_3px_rgba(0,0,0,0.08)] transition hover:-translate-y-px hover:shadow-[0_4px_12px_rgba(0,0,0,0.10)]">
      <div className="border-b border-[var(--blue)]/15 bg-[var(--blue)]/[0.04] px-5 py-2.5">
        <p className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wider text-[var(--blue)]">
      ...
      <div className="px-5 py-4">
        <h3 className="font-display text-lg font-semibold tracking-tight text-[var(--ink)]">{match.job.title}</h3>
            ...
            <ul className="mt-1.5 flex flex-wrap gap-1.5">
              {matchedRequired.map((skill) => (
                <li
                  ...
                  className="inline-flex items-center gap-1 rounded-full border border-[var(--verified)]/25 bg-[var(--verified)]/10 px-2.5 py-1 text-[11px] font-medium text-[var(--verified)]"
                >
        ...
        <p className="mt-3 text-xs leading-relaxed text-[var(--slate)]">{match.reasoning}</p>

        <p className="mt-3.5 border-l-2 border-[var(--rule)] pl-3 text-[13px] leading-relaxed text-[var(--slate)]">
          Your evidence profile is your application.
          <br />
          <span className="text-[var(--muted)]">One click. No forms. No resume upload.</span>
        </p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <Link
            ...
            className="rounded-xl border border-[var(--rule)] px-4 py-2.5 text-[13px] font-semibold text-[var(--ink)] transition hover:bg-[var(--panel-raised)]"
          >
            View details
          </Link>
        </div>
